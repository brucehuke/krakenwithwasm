...
#if WASM_ENABLE_INTERP != 0
    if (module_inst->module_type == Wasm_Module_Bytecode)
        count = wasm_list_function(
            (const WASMModuleInstance *)module_inst, func_name,param_count);
    return count;
#endif
#if WASM_ENABLE_AOT != 0
    if (module_inst->module_type == Wasm_Module_AoT)
        count = aot_list_function(
            (const AOTModuleInstance *)module_inst, func_name, param_count);
    return count;
#endif
