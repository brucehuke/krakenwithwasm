...
aot_list_function(const AOTModuleInstance *module_inst, char name[][512],int *param_count)
{
    uint32 i;
    int count = 0;
    printf("aot_list_function count: %d\n", module_inst->export_func_count );
    fflush(stdout);
    ...
    for (i = 0; i < module_inst->export_func_count; i++)
    {
        printf("aot_list_function count: %d,%s,%d\n", i,export_funcs[i].func_name, export_funcs[i].u.func.func_type->param_count);
        fflush(stdout);
        strcpy(name[count],export_funcs[i].func_name);
        param_count[count++] = export_funcs[i].u.func.func_type->param_count;
    }
    return count;
}
